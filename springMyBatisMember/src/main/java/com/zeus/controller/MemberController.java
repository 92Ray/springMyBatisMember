...
public class MemberController {
	...
	public String boardDetail(Member m, Model model) {
		...
		try {
			...
			if (member == null) {
				model.addAttribute("message", "%d님의 상세정보가 없습니다.".formatted(m.getNo()));
				return "member/failed";
			}
	...
	public String updatemember(Member m, Model model) {
		...
		try {
			...
			if(count>0) {
				model.addAttribute("message", "%s님의 회원정보 수정완료".formatted(m.getName()));
				return "member/success";
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		model.addAttribute("message", "%s 님의 회원정보 수정실패.".formatted(m.getName()));
		return "member/failed";
	}
